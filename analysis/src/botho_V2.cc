...
void Botho::init(TbConfig &config) {
	...
	h_hitmap_all = new TH2D("", ";column;row",
			n_cols+4, -2, n_cols+2, n_rows+4, -2, n_rows+2);
...
void Botho::event(const TbConfig &config, const Event &event) {
	...
	// increase number of raw tracks
	n_tracks_raw++;
	
	// raw hits
	h_rawHits->Fill(event.trackX, event.trackY);
...
void Botho::finalize(const TbConfig &config) {
	...
	// ToT module maps
	config.drawAndSave(this->name, "module_tot_sum", h_module_tot_avg);
	h_module_tot_avg->Divide(h_module_tot_hits);	// calculate average ToT
	config.drawAndSave(this->name, "module_tot_hits", h_module_tot_hits);
	config.drawAndSave(this->name, "module_tot_avg", h_module_tot_avg);
	
	// ToT pixel maps
	config.drawAndSave(this->name, "pixel_tot_sum", h_pixel_tot_avg);
	h_pixel_tot_avg->Divide(h_pixel_tot_hits);	// calculate average ToT
	config.drawAndSave(this->name, "pixel_tot_hits", h_pixel_tot_hits);
	config.drawAndSave(this->name, "pixel_tot_avg", h_pixel_tot_avg);
	
	// alternative ToT pixel maps
	config.drawAndSave(this->name, "pixel_tot_sum_alt", h_pixel_tot_avg_alt);
	h_pixel_tot_avg_alt->Divide(h_pixel_tot_hits_alt);	// calculate average ToT
	config.drawAndSave(this->name, "pixel_tot_hits_alt", h_pixel_tot_hits_alt);
	config.drawAndSave(this->name, "pixel_tot_avg_alt", h_pixel_tot_avg_alt);
...
//	drawPixelNicely(config, "pixel_tot_avg_alt_coarse_nice", h_pixel_tot_avg_alt);
	config.drawAndSave(this->name, "pixel_tot_avg_alt_coarse", h_pixel_tot_avg_alt);

	// 3D tot scatter histogram
	config.drawAndSave(this->name, "pixel_tot_3d", h_pixel_tot_3d);
...
//			<< double(anyhit) / double(ntracks) << endl;
	...
	config.drawAndSave(this->name, "trackModuleMap", h_trackModuleMap);
	config.drawAndSave(this->name, "hitModuleMap", h_hitModuleMap);
	...
	config.drawAndSave(this->name, "effModuleMap", h_effModuleMap);
	...
	config.drawAndSave(this->name, "trackPixelMap", h_trackPixelMap);
	config.drawAndSave(this->name, "hitPixelMap", h_hitPixelMap);
	config.drawAndSave(this->name, "effPixelMap", h_effPixelMap);
	delete h_effPixelMap;
	
	
	// control plots
	config.drawAndSave(this->name, "cuts_single", h_cuts_single);
	config.drawAndSave(this->name, "cuts_combined", h_cuts_combined);
	config.drawAndSave(this->name, "cuts_botho_single", h_cuts_botho_single);
	config.drawAndSave(this->name, "cuts_botho_combined", h_cuts_botho_combined);
