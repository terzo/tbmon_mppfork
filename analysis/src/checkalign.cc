...
void CheckAlign::event(const TbConfig &config, const Event& event) {
	...
	if (doCuts == true) {
		...
		if (event.fTrack != event::kGood) {
			h_cuts->Fill(0);	// FILL DEBUG HISTOGRAM
			return;
		}
		if (event.fTrackRegion != event::kGood) {
			h_cuts->Fill(1);	// FILL DEBUG HISTOGRAM
			return;
		}
	}

	for (vector<event::cluster_t>::const_iterator iClu = event.clusters.begin(); iClu != event.clusters.end(); iClu++) 
	{
//		if (event.clusters.size() == 1 and cluster::getSumTot(*iClu) >= 3) {
		if(cluster::getSumTot(*iClu) < 3) {
			h_cuts->Fill(2);	// FILL DEBUG HISTOGRAM
		}
//		if (cluster::isMatch(*iClu, event))

		//Stefano
		/*if (cluster::getSumTot(*iClu) >= 3) {
			h_resX->Fill(event.trackX - cluster::getEtaCorrectedX(*iClu, event,event.dut));
			h_resY->Fill(event.trackY - cluster::getEtaCorrectedY(*iClu, event,event.dut));
		}*/
		
		
		// DEBUG FILL
		if((cluster::isMatch(*iClu, event) == 0) && (event.fTrackRegion != event::kGood)) {
			h_cuts->Fill(4);	// FILL DEBUG HISTOGRAM
		}	// END OF DEBUG FILL
		if (doCuts == true) {
			if (cluster::isMatch(*iClu, event) == 0) {
				h_cuts->Fill(3);	// FILL DEBUG HISTOGRAM
				return;
			}
		}
		if (cluster::getSumTot(*iClu) == 0) {
			h_cuts->Fill(5);	// FILL DEBUG HISTOGRAM
			return;
		}
		...
		if (!(doCuts == true and event.ndof >= 2))
			h_cuts->Fill(8);	// FILL DEBUG HISTOGRAM
...
void CheckAlign::finalizeRun(const TbConfig &config) {
	...
	if (fabs(h_resX->GetMean()) > 100 or fabs(h_resY->GetMean()) > 100) {
...
void CheckAlign::finalize(const TbConfig &config) {
	...
	//DEBUG HISTOGRAM
	h_cuts->GetXaxis()->SetBinLabel(1, "event.fTrack != event::kGood");
	h_cuts->GetXaxis()->SetBinLabel(2, "event.fTrackRegion != event::kGood");
	h_cuts->GetXaxis()->SetBinLabel(3, "cluster::getSumTot(*iClu) < 3");
	h_cuts->GetXaxis()->SetBinLabel(4, "cluster::isMatch(*iClu, event) == 0");
	h_cuts->GetXaxis()->SetBinLabel(5,
			"(cluster::isMatch(*iClu, event) == 0) && (event.fTrackRegion != event::kGood)");
	h_cuts->GetXaxis()->SetBinLabel(6, "cluster::getSumTot(*iClu) == 0");
	h_cuts->GetXaxis()->SetBinLabel(7, "hitX < event.dut->getSkipCols() or \
		hitX > (event.dut->getNcols() - event.dut->getSkipCols())");
	h_cuts->GetXaxis()->SetBinLabel(8, "hitY < event.dut->getSkipRows() or \
		hitY > (event.dut->getNrows() - event.dut->getSkipRows())");
	h_cuts->GetXaxis()->SetBinLabel(10, "ALL EVENTS");
	config.drawAndSave(this->name, "cuts", h_cuts);
...
/*	TF1 *pol1_xx = new TF1("pol1_xx", "pol1", 3, 15);
	...
	TF1 *pol1_xx = new TF1("pol1_xx", "pol1", 3, 77);
	h_hitxVresx->Fit("pol1_xx", "+ QR");
	TF1 *pol1_yx = new TF1("pol1_yx", "pol1", 15, 320);
	h_hityVresx->Fit("pol1_yx", "+ QR");
	TF1 *pol1_xy = new TF1("pol1_xy", "pol1", 3, 77);
	h_hitxVresy->Fit("pol1_xy", "+ QR");
	TF1 *pol1_yy = new TF1("pol1_yy", "pol1", 15, 320);
	h_hityVresy->Fit("pol1_yy", "+ QR");
	//Pipe out the plots
